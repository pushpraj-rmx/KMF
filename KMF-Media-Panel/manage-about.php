...
if (isset($_GET['del'])) {
	...
	$slider_id = $_GET['del'];
	$query = mysqli_query($conn, "delete from about where about_id=$slider_id");
	if ($query) {
...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
	...
	<div class="d-flex flex-column flex-root">
					...
					<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
						<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
							...
							<div class="d-flex align-items-center flex-wrap mr-2">
								<!--begin::Page Title-->
								<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Manage About</h5>
								<!--end::Page Title-->
							...
							<div class="card card-custom gutter-b">
								<div class="card-header flex-wrap py-3">
									...
									<div class="card-toolbar">
										...
										<a href="<?= BASE_URL ?>add-about.php" class="btn btn-primary font-weight-bolder">
											<span class="svg-icon svg-icon-md">
												<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
													<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
														<rect x="0" y="0" width="24" height="24"></rect>
								...
								<div class="card-body">
									<table id="example" class="table table-bordered  dataTable " style="width:100%">
										...
										<tbody>
											...
											<?php
											$query = mysqli_query($conn, "select * from about order by about_id asc");
											...
											?>
													<tr>
														<td><?= $id ?></td>
														<td><?= $row['about_position'] ?></td>
														<td><?= $row['about_name'] ?></td>
														<td><?= $row['about_desc'] ?></td>
														<td><img src="<?= BASE_URL_IMG . 'about-image/' . $row['about_img'] . '' ?>" width="100"></td>
		...
		<?= ($row['is_active'] == 1 ? 'label-light-success' : 'label-light-danger') ?> label-inline"><?= ($row['is_active'] == 1 ? 'Active' : 'Inactive') ?></span></td>
														<td><a href="<?= BASE_URL ?>edit-about.php?edit_rec=<?= $row['about_id'] ?>" class="btn btn-warning">Edit</a></td>

														<td><a href="<?= BASE_URL ?>manage-about.php?del=<?= $row['about_id'] ?>" class="btn btn-danger del_rec" data-url="<?= BASE_URL ?>manage-about.php?del=<?= $row['about_id'] ?>">Delete</a></td>
													</tr>
												<?php $id++;
												}
											} else { ?>
