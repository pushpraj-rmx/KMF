...
if (isset($_GET['del'])) {
	...
	$slider_id = $_GET['del'];
	$query = mysqli_query($conn, "delete from slider where slider_id=$slider_id");
	if ($query) {
...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
	...
	<div class="d-flex flex-column flex-root">
								...
								<div class="card-body">
									<table id="example" class="table table-bordered  dataTable " style="width:100%">
										...
										<tbody>
											...
											<?php
											$query = mysqli_query($conn, "select * from slider order by slider_id asc");
											...
											?>
													<tr>
														<td><?= $id ?></td>
														<td><?= $row['slider_preheader'] ?></td>
														<td><?= $row['slider_header'] ?></td>
		...
		<?= ($row['is_active'] == 1 ? 'label-light-success' : 'label-light-danger') ?> label-inline"><?= ($row['is_active'] == 1 ? 'Active' : 'Inactive') ?></span></td>
														...
														<td><a href="<?= BASE_URL ?>manage-slider.php?del=<?= $row['slider_id'] ?>" class="btn btn-danger del_rec" data-url="<?= BASE_URL ?>manage-slider.php?del=<?= $row['slider_id'] ?>">Delete</a></td>
													</tr>
												<?php $id++;
												}
											} else { ?>
