...
if (isset($_GET['del'])) {
	...
	$slider_id = $_GET['del'];
	$query = mysqli_query($conn, "delete from youtube_package where ypackage_id=$slider_id");
	if ($query) {
		header('Location:' . BASE_URL . 'manage-youtube-packages.php');
	}
}
...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
	...
	<div class="d-flex flex-column flex-root">
								...
								<div class="card-body">
									<table id="example" class="table table-bordered  dataTable " style="width:100%">
										...
										<tbody>
											...
											<?php
											$query = mysqli_query($conn, "select * from youtube_package order by ypackage_id asc");
		...
		<?= ($row['is_active'] == 1 ? 'label-light-success' : 'label-light-danger') ?> label-inline"><?= ($row['is_active'] == 1 ? 'Active' : 'Inactive') ?></span></td>
														<td><a href="<?= BASE_URL ?>edit-youtube-packages.php?edit_rec=<?= $row['ypackage_id'] ?>" class="btn btn-warning">Edit</a></td>

														<td><a href="<?= BASE_URL ?>manage-youtube-packages.php?del=<?= $row['ypackage_id'] ?>" class="btn btn-danger del_rec" data-url="<?= BASE_URL ?>manage-youtube-packages.php?del=<?= $row['ypackage_id'] ?>">Delete</a></td>
													</tr>
												<?php $id++;
												}
											} else { ?>
