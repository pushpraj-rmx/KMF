...
if (isset($_GET['del'])) {
	...
	$slider_id = $_GET['del'];
	$query = mysqli_query($conn, "delete from spotify_package where spackage_id=$slider_id");
	if ($query) {
		header('Location:' . BASE_URL . 'manage-spotify-packages.php');
	}
}
...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
	...
	<div class="d-flex flex-column flex-root">
					...
					<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
						<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
							...
							<div class="d-flex align-items-center flex-wrap mr-2">
								<!--begin::Page Title-->
								<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Manage Spotify Packages</h5>
								<!--end::Page Title-->
								...
								<div class="card-body">
									<table id="example" class="table table-bordered  dataTable " style="width:100%">
										...
										<tbody>
											...
											<?php
											$query = mysqli_query($conn, "select * from spotify_package order by spackage_id asc");
		...
		<?= ($row['is_active'] == 1 ? 'label-light-success' : 'label-light-danger') ?> label-inline"><?= ($row['is_active'] == 1 ? 'Active' : 'Inactive') ?></span></td>
														<td><a href="<?= BASE_URL ?>edit-spotify-packages.php?edit_rec=<?= $row['spackage_id'] ?>" class="btn btn-warning">Edit</a></td>

														<td><a href="<?= BASE_URL ?>manage-spotify-packages.php?del=<?= $row['spackage_id'] ?>" class="btn btn-danger del_rec" data-url="<?= BASE_URL ?>manage-spotify-packages.php?del=<?= $row['spackage_id'] ?>">Delete</a></td>
													</tr>
												<?php $id++;
												}
											} else { ?>
