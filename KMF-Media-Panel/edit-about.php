...
if (isset($_POST['update_slider'])) {
	...
	if (!empty($_FILES['about_img']['name'])) {
		$about_img = 'img_' . rand(0, 1000) . '_' . $_FILES['about_img']['name'];
	} else {
		$about_img = $_POST['about_img_hidden'];
	}

	$escapedDescription = mysqli_real_escape_string($conn, $about_desc);
	$about_desc = $escapedDescription;

	$query = mysqli_query($conn, "UPDATE about
                                   SET about_position='$about_position',
                                       about_name='$about_name',
                                       about_desc='$about_desc',
                                       about_img='$about_img',
                                       is_active=$is_active
                                   WHERE about_id=$edit_id");

	if ($query) {
		if (!empty($_FILES['about_img']['name'])) {
			move_uploaded_file($_FILES['about_img']['tmp_name'], 'images/about-image/' . $about_img . '');
			unlink('images/about-image/' . $_POST['about_img_hidden']);
		}
		header('Location: manage-about.php');
		exit(); // Always exit after a header redirect
	}
}
$details = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM about WHERE about_id=$edit_id"));
?>
<!--begin::Body-->
					...
					<div class="subheader py-2 py-lg-4 subheader-solid" id="kt_subheader">
						<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
							...
							<div class="d-flex align-items-center flex-wrap mr-2">
								<!--begin::Page Title-->
								<h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Edit About</h5>
								<!--end::Page Title-->
										...
										<form action="" enctype="multipart/form-data" method="post" id="edit-slider">
											<div class="card-body">
												...
												<div class="form-group">
													...
													<input type="text" name="about_position" value="<?= $details['about_position'] ?>" class="form-control" autocomplete="off" placeholder="Heading">
												</div>

												<div class="form-group">
													<label for="exampleInputPassword1">About Name
														<span class="text-danger">*</span></label>
													<input type="text" name="about_name" value="<?= $details['about_name'] ?>" class="form-control" autocomplete="off" placeholder="Heading">
												</div>

												<div  class="form-group">
													<label for="exampleInputPassword1">About Description
														<span class="text-danger">*</span></label>
													<textarea name="about_desc" id="editor" cols="30" rows="10"><?= $details['about_desc'] ?></textarea>
												</div>
												...
												<div class="form-group">
													...
													<?php if (!empty($details['about_img'])) { ?>
														<img src="<?= BASE_URL_IMG ?>about-image/<?= $details['about_img'] ?>" width='50' class="mt-3">
													<?php } ?>
													<input type="hidden" name="about_img_hidden" value="<?= $details['about_img'] ?>">
												</div>
